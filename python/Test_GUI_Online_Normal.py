...
class OnlineTestGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("S7RTT Normal Mode (Triggered Planning)")
    ...
    def _init_ui(self):
        ...
        target_frame = ttk.LabelFrame(control_panel, text="Target Settings")
        target_frame.pack(fill=tk.X, pady=10)
        
        self.var_target_p = self._create_input(target_frame, "Target Pos:", 100.0)
        self.var_target_v = self._create_input(target_frame, "Target Vel:", 0.0)
        
        self.btn_apply = ttk.Button(target_frame, text="Apply New Target", command=self._on_apply_target)
        self.btn_apply.pack(fill=tk.X, padx=5, pady=10)
        ...
        self.lbl_status = ttk.Label(stats_frame, text="Status: Idle", foreground="gray")
        self.lbl_status.pack(anchor="w", padx=5, pady=5)
    ...
    def _on_apply_target(self):
        try:
            ...
            print(f"Planning: Current P={self.current_state.p:.2f} -> Target P={tgt_p:.2f}")

            new_traj = self.planner.plan(
                self.current_state, 
                tgt_p, 
                tgt_v, 
                v_max, a_max, j_max
            ...
            if new_traj:
                self.current_traj = new_traj
                self.traj_local_time = 0.0
                self.lbl_status.config(text="Status: Moving", foreground="green")
            else:
    ...
    def _run_cycle(self):
        ...
        try:
            if self.current_traj:
                self.traj_local_time += self.SIM_DT
                
                next_state = self.planner.at_time(self.current_traj, self.traj_local_time)
                self.current_state = next_state
                
                if abs(self.current_state.v) < 1e-6 and \
                   abs(self.current_state.a) < 1e-6 and \
                   self.traj_local_time > 0.1:
                   self.lbl_status.config(text="Status: Done", foreground="blue")
